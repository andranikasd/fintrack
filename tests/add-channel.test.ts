...
describe('/add channel workflow', () => {
  it('asks for an account and then creates a daily channel table', async () => {
    ...
    const message=sent.find(x=>x.method==='sendMessage'&&x.payload?.chat_id===-1001); expect(message?.payload?.text).toContain('metro @ Card | 150'); expect(message?.payload?.text).toContain('Total: 150');
  });
  it('edits an existing source post in place and preserves its existing rows', async () => {
    ...
    const edit=calls.find(x=>x.method==='editMessageText');expect(edit?.payload?.chat_id).toBe(-1001);expect(edit?.payload?.message_id).toBe(77);expect(edit?.payload?.text).toContain('coffee @ Card | 200');expect(edit?.payload?.text).toContain('metro @ Card | 150');expect(edit?.payload?.text).toContain('Total: 350');
  });
