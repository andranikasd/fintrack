...
describe('production SQLite storage', () => {
  it('migrates once, persists across restarts, and stores exact savings', async () => {
    ...
    const db = open(path);
    expect(db.migrate('migrations')).toEqual(['0001_init.sql', '0002_channel_savings.sql', '0003_income.sql', '0004_dashboard.sql', '0005_accounts.sql', '0006_dashboard_workspace.sql']);
