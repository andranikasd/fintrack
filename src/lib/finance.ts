...
export async function financialStatus(db: Db, user: number, today: string) {
  ...
  const [goals,prefs,budget,spent,saved,contributions] = await Promise.all([
    db.finance.goals(user),db.finance.preferences(user),db.budget(user,0),db.totalBetween(user,from,to),
    db.finance.savingsTotal(user,from,to),db.finance.goalSavingsOn(user,today),
  ]);
  ...
  return {goals,prefs,budget,spent,saved,available,plans};
}
