...
async function fixture(){const {db,d1}=testDb();await db.ensureUser(1);await db.ensureUser(2);const today=todayIn(tz);const act=(body:Record<string,unknown>,user=1)=>dashboardAction(db,d1,user,tz,{requestId:crypto.randomUUID(),...body});return {db,d1,act,today};}
describe('dashboard workspace',()=>{
 ...
 it('recovers manual edits and deletions including exact amounts and account assignment',async()=>{
  ...
  await act({action:'edit',kind:'income',id:income.id,label:'Bonus corrected',amount:'200.12',accountId:account.id,expected:{day:today,label:'Bonus',amountMinor:10077}});
  ...
  await act({action:'delete',kind:'income',id:income.id,expected:{day:today,label:'Bonus',amountMinor:10077}});history=(await db.workspace.history(1)).rows.find(r=>r.entity==='income'&&r.operation==='delete')!;
 ...
 it('closes a checked month, detects later changes and preserves monetary balances during checks',async()=>{
  const {db,d1}=await fixture(),day='2026-01-01',today='2026-02-02';await d1.prepare("INSERT INTO accounts(user_id,name,opening_minor,opening_on) VALUES(1,'Card',10000,?)").bind(day).run();const a=(await db.accounts.list(1,today))[0]!;
 ...
 it('stores notes/receipts privately, rejects invalid files, and retains notes through deletion and undo',async()=>{
  ...
  await act({action:'delete',kind:'expense',id,expected:{day:today,label:'item',amountMinor:10000}});const history=(await db.workspace.history(1)).rows[0]!;await act({action:'history-undo',id:history.id});data=await dashboardData(db,1,tz,today,today);expect(data.workspace.attachments[0]!.entry_key).toBe(data.records[0]!.entryKey);
 });
