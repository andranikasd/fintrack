...
describe('SQLite sync and savings invariants',()=> {
  it('replaces only that post, handles duplicates/stale edits, date changes and row removal',async()=> {
    const {db}=testDb();await db.ensureUser(1); await db.accounts.create(1,'Test account',100000000,'2000-01-01','test-account:1');
  ...
  it('opening balance is not daily savings; confirmed transfers are idempotent and owner-scoped',async()=> {
    const {db}=testDb();await db.ensureUser(1); await db.accounts.create(1,'Test account',100000000,'2000-01-01','test-account:1');await db.finance.putGoal(1,'laptop',96038177,null,300000,20000000,null);
    const g=(await db.finance.goals(1))[0]!;
  ...
  it('allocates shared budget across goals without overspending and subtracts confirmed transfers',async()=> {
    const {db}=testDb();await db.ensureUser(1); await db.accounts.create(1,'Test account',100000000,'2000-01-01','test-account:1');await db.setBudget(1,0,16000);await db.finance.setFunding(1,'shared',0);
...
it('routes channel updates without a sender, prevents unlinked channels, and survives malformed edits',async()=> {
  const {db,d1}=testDb();await db.ensureUser(1); await db.accounts.create(1,'Test account',100000000,'2000-01-01','test-account:1');await db.finance.link(-1001,1,'My finances');
  ...
  expect(await db.totalBetween(1,'2026-09-14','2026-09-14')).toBe(4310);expect(sent).toHaveLength(2);
  expect(JSON.stringify(sent[0])).toContain('Which category fits');
  expect(JSON.stringify(sent[1])).toContain('last valid records are preserved');
});
