...
(async()=>{
  ...
  check(document.querySelector('.wallet-manage').textContent==='View accounts','Static report implies account editing');
  check(!document.getElementById('source-help').textContent.includes('live dashboard'),'Static help points to removed editor');
  check(document.getElementById('selection-summary').textContent.includes('record'),'Filter context missing');
  document.getElementById('reset').click();
  const count=Number(document.getElementById('row-count').textContent.split(' ')[0]);
  document.getElementById('page-next').click();const originalPage=document.getElementById('row-count').textContent;
  window.dispatchEvent(new Event('beforeprint'));
  check(document.querySelectorAll('#rows tr').length===count,'Print omits ledger rows after the current page');
  check(document.querySelector('.analysis-disclosure').open,'Print omits detailed analysis');
  check(document.getElementById('print-summary').textContent.includes('Snapshot as of'),'Print has no report date');
  window.dispatchEvent(new Event('afterprint'));
  check(document.getElementById('row-count').textContent===originalPage,'Printing lost the current ledger page');
  check(!document.querySelector('.analysis-disclosure').open,'Printing changed the disclosure state');
  document.getElementById('search').value='nothing matches this fixture';document.getElementById('search').dispatchEvent(new Event('input'));
  check(document.getElementById('rows').textContent.includes('Clear your filters'),'Empty filter results have no recovery guidance');
  document.getElementById('reset').click();
